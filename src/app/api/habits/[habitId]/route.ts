...
// DELETE /api/habits/:habitId?user_id=...
export async function DELETE(req: Request, { params }: { params: { habitId: string } }) {
  try {
    const { searchParams } = new URL(req.url);
    const user_id = searchParams.get('user_id') || 'web';

    const url = new URL(`/api/habits/${encodeURIComponent(params.habitId)}`, getBase());
    ...
    console.error('DELETE /api/habits/[habitId] error:', e);
