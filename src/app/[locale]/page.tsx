...
type Card = { titleKey: string; textKey: string };
...
export default async function LocaleHome() {
  const t = await getTranslations(); // серверный API
    ...
    <section className="mx-auto max-w-5xl text-center space-y-10">
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {CARDS.map(c => (
            <div key={c.titleKey} className="rounded-2xl border border-white/10 p-5 text-left">
              <h3 className="text-base font-medium">{t(c.titleKey)}</h3>
              <p className="mt-2 text-sm text-zinc-300">{t(c.textKey)}</p>
            </div>
          ))}
        </div>
      </section>
